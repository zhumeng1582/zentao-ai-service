...
| POST | `/api/v1/conversations/{conversation_id}/messages/{message_id}/stop` | `chat:send` | MVP |
...
- `HTTP_METHOD` 使用大写 ASCII。
- `NORMALIZED_PATH` 必须以 `/` 开头；拒绝 `.`、`..` 路径段和重复 `/`，不做会改变语义的自动修正。
- Path 的每个 UTF-8 段按 RFC 3986 编码，未保留字符 `A-Z a-z 0-9 - . _ ~` 不编码，分隔符 `/` 保留，百分号十六进制使用大写。
- Query 先解析为名称和值的有序对，保留重复参数和空值；名称和值分别按 RFC 3986 编码。
- 编码后的 Query 按“参数名升序、参数值升序”稳定排序，再以 `name=value` 和 `&` 拼接。
- 空值写成 `name=`；没有 Query 时使用空字符串。
- 空格编码为 `%20`，字面加号编码为 `%2B`，不得使用表单编码中的 `+` 表示空格。
- 签名必须使用实际传输的原始 Body 字节计算 `BODY_SHA256`；JSON 不得在签名后重新格式化。
...
raw_query     = tag=a%2Bb&scope_type=doc&limit=20&tag=a%20b
canonical_query = limit=20&scope_type=doc&tag=a%20b&tag=a%2Bb
body          = <empty bytes>
body_sha256   = e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
tenant_id     = company_001
client_id     = zentao_company_001
actor_type    = user
zentao_user   = pm01
timestamp     = 1783650600
nonce         = 5f7e3198-0a92-4dee-96fa-c12bb34fd509
signature_hex = c479a693ce68eeddd5398c83c602b79a09a3e4ec83b9189bbc69abf73bee656e
```

PHP 与 Python 实现必须用该测试向量得到相同签名后才能联调业务接口。
...
任一已启用功能的必需依赖不可用时返回 `503`。启用 `feedback` 会话时 `zentao_callback` 必须完成双向 HMAC 探测；启用 `help` 会话时 `help_kb` 必须存在已发布 Manifest 和当前可查询集合。探测不得实际读取业务对象或调用付费模型。
...
- AI 服务使用独立的 `callback_client_id` 和 `callback_client_secret`，不能复用禅道调用 AI 服务的 Client 密钥。
- 请求头、Canonical Request、Query 标准化、Nonce、防重放和密钥轮换完全复用第 4 章 HMAC 规范。
- 禅道将 Callback Client 绑定到单一租户和最小 Scope，例如 `zentao:ping`、`zentao:authorize`、`zentao:object:read`、`zentao:file:read`、`zentao:writeback`。
- Callback 请求中的用户账号只是待模拟校验的 Actor，不能替代 Callback Client 鉴权，也不能获得超过该用户原有禅道权限的访问范围。
- 部署配置统一使用 Callback Client，不再使用语义不明确的通用 `ZENTAO_SERVICE_TOKEN`。
...
Callback Scope：`zentao:ping`。请求使用完整 HMAC，但不携带业务用户、不读取业务数据。
...
```json
{
  ...
  "data": {
    "status": "ok",
    "zentao_version": "22.3",
    "server_time": "2026-07-10T12:00:00+08:00"
  }
}
```

AI 服务 `/readyz` 使用该接口验证反向网络、时钟和 Callback Client，不得用真实权限接口作为健康探测。

### 18.1 批量权限校验

```http
POST /api/ai/v1/authorize/batch
```
...
Callback Scope：`zentao:object:read`。
...
```json
{
  ...
  "data": {
    ...
    "items": [
      {
        "type": "feedback",
        "id": "456",
        "source_revision": "2026-07-10T10:20:30+08:00",
        "fields": {
          "product": "3",
          "module": "12",
          "type": "bug",
          "title": "设备升级后无法连接",
          "desc": "升级到 2.3.1 后设备无法连接。",
          "pri": "2",
    ...
    "denied": [
      {"type": "feedback", "id": "999", "reason": "OBJECT_DENIED"}
    ]
  }
}
```
...
- 禅道在读取字段的同一次请求中完成对象级权限检查，避免“先鉴权、后读取”之间权限变化。
- MVP 支持 `feedback`；后续可复用于需要实时读取的 Bug、文档和其他对象。
- `field_profile` 由禅道维护白名单，首批只支持 `ai_chat`；调用方不能请求任意数据库字段。
- `denied` 只返回请求中的类型、ID 和通用原因，不返回标题、正文或关联信息。
- 不存在且调用方无权知道其存在的对象统一按 denied/not-found 模糊处理。
- 反馈会话每次生成回答前读取当前版本；关联 Bug、文档仍需分别通过对象级权限校验后才能检索或引用。
- 返回内容视为不可信数据，不能覆盖 System Prompt 或扩大 Conversation Scope。

反馈详情页 `feedback` 会话通过本接口获得当前反馈内容，不要求将全部反馈同步或写入向量库。

### 18.3 获取附件

```http
GET /api/ai/v1/files/{file_id}/content
```
...
- 禅道 Callback 内部接口使用 `/api/ai/v1` 独立版本前缀；反向契约发生不兼容变化时升级为 `/api/ai/v2`。
- 首批禅道字段契约固定为开源版 22.3，字段映射变化通过独立模板/Profile 版本演进，不能静默改变已发布字段语义。
...
- 反馈会话通过内部批量取数接口读取当前反馈白名单字段，权限检查和读取在同一请求完成。
- 流式中断后可以通过历史消息接口恢复最终状态。
- SSE 网络断开不等于停止；显式 Stop 接口使消息进入 `stopped`，重复停止保持幂等。
...
本章同时定义首批 MVP 的全局 AI 客服会话和 V1.1 的通用推荐接口。MVP 支持 `help`、`bug`、`doc`、`feedback` 范围的多轮聊天、流式回答和历史消息；项目问答、混合范围和通用跨类型推荐在 V1.1 或后续版本实现。所有入口复用相同的检索、权限、引用和消息存储能力。

全局入口约定：

- 登录后的禅道业务页面右下角展示机器人图标，点击后在当前页面打开聊天弹窗或抽屉，不刷新主页面。
- Bug 详情页由禅道服务端绑定 `bug` 范围，文档详情页绑定 `doc` 范围，反馈详情页绑定 `feedback` 范围，其他页面绑定 `help` 范围。
...
| `feedback` | 围绕指定反馈读取当前反馈字段和已关联的授权资料 | MVP |
...
```json
{
  ...
  "scope": {
    ...
    "object_type": "feedback",
    "object_id": "456",
    "allowed_object_types": ["feedback", "bug", "doc"]
  },
  "metadata": {
    "entry_page": "feedback-view"
  }
}
```

MVP 的 `feedback` 会话通过第 18.2 节 `POST /api/ai/v1/source-objects/batch-get` 按当前反馈 ID 实时读取白名单字段并在同一请求中执行对象级权限校验，不要求先将全部反馈向量化，也不开放自动分类和生成 Bug/需求草稿。
...
创建 Bug、文档、反馈或项目会话时必须校验用户对绑定对象的读取权限。`metadata.entry_page` 只用于审计和产品分析，不作为授权依据；禅道扩展必须根据服务端当前路由构造并签名 `scope`。客户端不能提交自定义 System Prompt、模型 API Key 或绕过检索范围的指令。
...
- 网络断开、关闭弹窗和用户主动停止是不同事件；网络断开或关闭弹窗默认不停止服务端生成。
...
鉴权 Scope：`chat:send`，需要 `X-Idempotency-Key`。

规则：

- 只允许会话所有者停止该会话中状态为 `pending` 或 `generating` 的 Assistant 消息。
- 服务端先持久化 `stop_requested_at`，再尽力取消模型下游请求；无论供应商是否支持取消，都不得继续向用户推送新 Token。
- 已生成的部分文本可以保留，消息最终状态写为 `stopped`，未生成完整引用时不得伪造引用。
- 重复停止同一条 `stopped` 消息返回原结果，保证幂等。
- `completed`、`failed`、`restricted` 消息返回 `409 MESSAGE_NOT_STOPPABLE`。
- 客户端断开 SSE 不调用本接口；只有用户点击“停止生成”才调用。
...
```json
{
  ...
  "data": {
    ...
    "status": "stopped",
    "content": "已经生成的部分内容",
    "stopped_at": "2026-07-10T11:01:02+08:00"
  }
}
```
...
data: {"code":0,"msg":"success","data":{"message_id":"01JZMSG002"}}
```
...
| `40912` | `MESSAGE_NOT_STOPPABLE` | 409 | 消息当前状态不允许停止生成 |

### 25.14 新增验收项

- MVP 用户可以创建 `help`、文档、Bug 和反馈范围的会话；项目范围会话按 V1.1 验收。
- 连续追问不会扩大创建时的检索范围。
...
- Bug、文档、反馈页面自动绑定当前对象，其他页面仅使用隔离的平台帮助知识库。
- 平台帮助资料只从审核通过的 Manifest 发布，按语言、禅道版本和 Edition 过滤。
