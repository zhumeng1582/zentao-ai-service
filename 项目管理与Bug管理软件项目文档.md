...
第一阶段 AI 只做“Bug 与单文档辅助分析 + 全局 AI 客服 + Bug/反馈提交完整性校验 + 对象级权限校验 + 人工确认”。
...
| `submission-validation` | Bug、反馈固定规则、AI 完整性检查、策略判定和审计 |
...
POST /api/submission-validations
POST /api/submission-validations/{id}/outcome
...
第一版只做 2 个对象分析闭环、1 个全局 AI 客服入口、1 个提交质量控制闭环和一组必需的基础能力。
...
- 用户点击原提交按钮后，先执行必填、格式、无效文本和字段一致性等固定规则。
- 固定规则通过后，由 AI 检查描述质量、关键缺失项并生成字段级建议和补充问题。
- 服务端策略引擎输出 `pass`、`warn`、`block`、`degraded`，不能直接采用模型的放行结论。
- `warn` 可由用户确认提交；`block` 必须补充后重检，或由授权角色填写原因后强制提交。
...
AI 服务策略引擎执行固定内容规则
  ↓
固定规则通过后调用模型执行语义完整性检查
  ↓
策略引擎返回 pass / warn / block / degraded
  ↓
直接提交 / 确认提交 / 补充重检 / 授权强制提交
  ↓
保存业务对象并记录校验结果和最终处理状态
```
...
| `submission_validations` | Bug/反馈提交完整性校验、内容哈希、等级和结构化问题 |
| `submission_validation_outcomes` | 正常提交、修改后提交、强制提交或放弃的审计记录 |
...
│   ├── llm/             # 模型网关
│   ├── bug_ai/          # Bug 分析
│   ├── submission_validation/ # Bug/反馈提交完整性校验
│   ├── feedback_ai/     # 客户反馈分析
│   ├── doc_ai/          # 文档问答
│   ├── project_ai/      # 项目风险分析
│   └── report/          # 报告生成
...
- Bug 相似检索 API。
- Bug AI 分析 API。
- Bug、反馈提交完整性校验 API、策略引擎和校验审计表。
- 创建页字段提示、结果弹窗、重新校验和授权强制提交交互。
- 单文档摘要和问答 API。
- 全局 AI 客服悬浮入口与聊天弹窗。
...
- 上线 Bug、反馈提交固定规则和 AI 完整性校验，支持降级、内容哈希和强制提交审计。
- AI 结果采纳、驳回和仅添加评论的写回。
...
| 提交校验误判 | 正常 Bug/反馈被阻断 | 固定规则与 AI 分层、警告可确认、授权强制提交、AI 失败自动降级 |
