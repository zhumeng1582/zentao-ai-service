...
- Bug AI 分析。
- 文档摘要和文档问答。
- 全局 AI 客服中的平台帮助、当前 Bug、当前文档和当前反馈问答。
- 后续扩展项目范围知识问答。
- Bug 和文档图片 OCR 检索。
...
这里的“两类对象”指首批向量化对象。反馈详情页的 MVP 会话实时读取当前反馈字段并校验权限，不要求先将全部反馈向量化；反馈向量化、自动分类和生成 Bug/需求草稿仍属于 V1.1。
...
提交完整性校验边界：Bug、反馈创建页的提交前校验直接处理尚未落库的表单草稿，不进入本同步链路，也不写入 Qdrant；AI 服务仅保存内容哈希、结构化问题和审计摘要。详细实现以《Bug 与反馈提交完整性校验设计文档》为准。后续如增加疑似重复对象提示，可复用本文件定义的授权检索能力，但相似度不得作为硬阻断提交的唯一依据。
...
```text
用户点击 Bug AI 分析、文档问答或任一业务页面右下角机器人入口
  ↓
禅道扩展带当前用户身份调用 AI 服务
  ↓
...
- 在登录后的禅道业务页面右下角提供统一机器人入口和聊天弹窗。
- 在 Bug、文档详情页分别复用已索引的当前对象知识；反馈详情页实时读取当前反馈字段；其他页面只使用平台帮助知识库。
- 支持流式回答、连续追问、引用来源和历史恢复。
...
2. 全局 AI 客服的 `help`、`bug`、`doc`、`feedback` 会话与历史消息。
3. 客户反馈分类和 Bug/需求草稿。
4. 项目范围及跨对象问答和通用推荐。
5. 项目风险周报。
6. 模型管理后台、审计与用量页面。
