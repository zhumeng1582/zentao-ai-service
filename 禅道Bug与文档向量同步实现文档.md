...
- 在 Bug、文档详情页分别复用已索引的当前对象知识；其他页面只使用平台帮助知识库。
...
- Bug、文档会话不能通过追问扩大到其他对象；页面上下文由禅道服务端生成并签名。
- `help` 会话不能检索项目、Bug、文档等业务数据。
...
- `help`、`bug`、`doc` 范围支持连续追问、流式回答和历史恢复。
- Bug、文档页面自动绑定当前对象，其他页面只检索平台帮助知识。
...
2. 全局 AI 客服的 `help`、`bug`、`doc` 会话与历史消息。
3. 客户反馈分类和 Bug/需求草稿。
4. 项目范围及跨对象问答和通用推荐。
5. 项目风险周报。
6. 模型管理后台、审计与用量页面。
